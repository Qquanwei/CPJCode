...
#define IMAGE_SOURCE "/home/wenqing/pictures/erjiao.jpg"
#define IMAGE_OUTPUT "/home/wenqing/pictures/erjiao_enhance.jpg"
...
int main()
{
    ...
    const int dst_histSize=255;
    const float range[]={0,255};
    const float *histRange={range};


    src=imread(IMAGE_SOURCE);
  ...
  //  imshow("src",src);
   ...
   // imwrite("/home/wenqing/pictures/erjiao_gray.jpg",src_gray);
    ...
    const int hist_w=600;
    const int hist_h=600;
    ...
    imshow("dst",dst);
    imwrite(IMAGE_OUTPUT,dst);
    calcHist(&dst,1,0,Mat(),dst_hist,1,&histSize,&histRange,true,false);

    const int dst_hist_w=600;
    const int dst_hist_h=600;
    const int dst_bin_w=cvRound((double) hist_w/histSize);
